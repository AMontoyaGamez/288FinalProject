...
uint8_t button_getButton() {
    ...
    int button = 6;
    //Checks every button, from left to right, by shifting it to the fist place and masking it, until the value is 1 because it means the corresponding button is pressed.
    while(button > 0 && ((GPIO_PORTE_DATA_R >> (button - 1)) & 0b0001))
        button--;
	
	return button; // Returns the position of the leftmost button being pushed
}
